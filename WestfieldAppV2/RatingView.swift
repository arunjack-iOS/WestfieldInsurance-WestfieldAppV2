...
//  WestfieldAppV2
//
//  Created by Arun Jack on 03/01/18.
//  Copyright © 2018 Arun Jack. All rights reserved.
//
...
@objc public protocol RatingViewDelegate {
    ...
    @objc optional func updateRatings(_ ratingView: RatingView, didUpdate rating: Double)
    
    /// Returns the rating value as the user pans
    @objc optional func updateRatings(_ ratingView: RatingView, isUpdating rating: Double)
}
...
open class RatingView: UIView {
    ...
    private var unStaredImageViews: [UIImageView] = []
    
    private var staredImageViews: [UIImageView] = []
    
    /// Sets the empty image (e.g. a star outline)
    @IBInspectable open var emptyImage: UIImage? {
        didSet {
            // Update empty image views
            for imageView in unStaredImageViews {
    ...
    @IBInspectable open var fullImage: UIImage? {
        didSet {
            ...
            for imageView in staredImageViews {
    ...
    @IBInspectable open var type: RatingType = .fullRatings
    ...
    private func initImageViews() {
        guard unStaredImageViews.isEmpty && staredImageViews.isEmpty else {
            return
        }
        ...
        for _ in 0..<maxRating {
            ...
            unStaredImageViews.append(emptyImageView)
            addSubview(emptyImageView)
            
            let fullImageView = UIImageView()
            fullImageView.contentMode = imageContentMode
            fullImageView.image = fullImage
            staredImageViews.append(fullImageView)
    ...
    private func removeImageViews() {
        ...
        for i in 0..<unStaredImageViews.count {
            var imageView = unStaredImageViews[i]
            imageView.removeFromSuperview()
            imageView = staredImageViews[i]
            imageView.removeFromSuperview()
        }
        unStaredImageViews.removeAll(keepingCapacity: false)
        staredImageViews.removeAll(keepingCapacity: false)
    }
    
    // Refresh hides or shows full images
    private func refresh() {
        for i in 0..<staredImageViews.count {
            let imageView = staredImageViews[i]
    ...
    private func updateLocation(_ touch: UITouch) {
        ...
        for i in stride(from: (maxRating-1), through: 0, by: -1) {
            let imageView = unStaredImageViews[i]
        ...
        // Update delegate
        delegate?.updateRatings?(self, isUpdating: rating)
    }
    ...
    override open func layoutSubviews() {
        ...
        let desiredImageWidth = frame.size.width / CGFloat(unStaredImageViews.count)
        ...
        let imageXOffset = (frame.size.width - (imageViewSize.width * CGFloat(unStaredImageViews.count))) /
            CGFloat((unStaredImageViews.count - 1))
    ...
    override open func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        // Update delegate
        delegate?.updateRatings?(self, didUpdate: rating)
    }
